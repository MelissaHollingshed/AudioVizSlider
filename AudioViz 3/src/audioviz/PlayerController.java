...
public class PlayerController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        visualizers.stream().map((visualizer) -> {
        ...
        for (Integer bands : bandsList) {
          ...
          timeSlider.valueProperty().addListener((ObservableValue<? extends Number> ov, Number oldValue, Number newValue) -> {
              if (mediaPlayer != null) {
    ...
    private void handleUpdate(double timestamp, double duration, float[] magnitudes, float[] phases) {
        ...
        currentText.setText(display(Double.toString(ms)));
        if (counter++ == 200) {
